...
<form method="POST" action="index.php?page=reglages&action=doreglages">
	...
	<div class="cadre">
		...
		<div class="row">
			...
			<div class="five columns">
				<label>Qualité de l'image (0-100):</label><div id="texteQualite" ><?= $settingsInterface -> picture_quality ?></div>
				<input class="u-full-width" type="range" min="0" max="100" value="<?= $settingsInterface -> picture_quality ?>" placeholder="<?= $settingsInterface -> picture_quality ?>" name="picture_quality" id="picture_quality" oninput="updateQualite(this.value);">
			</div>
		</div>
		...
		<div class="five columns">
			<label for="timelapse_mode">fréquence de sauvegarde du film:</label>
			<select class="u-full-width" id="timelapse_mode" name="timelapse_mode">
				<option value="hourly" <?= ($settingsInterface -> timelapse_mode=='hourly') ? 'selected' : "" ?> >Toute les heures</option>
				<option value="daily" <?= ($settingsInterface -> timelapse_mode=='daily') ? 'selected' : "" ?> >Tous les jours</option>
				<option value="weekly-sunday" <?= ($settingsInterface -> timelapse_mode=='weekly-sunday') ? 'selected' : "" ?> >Toutes les semaines (dimanche)</option>
				<option value="weekly-monday" <?= ($settingsInterface -> timelapse_mode=='weekly-monday') ? 'selected' : "" ?> >Toutes les semaines (lundi)</option>
				<option value="monthly" <?= ($settingsInterface -> timelapse_mode=='monthly') ? 'selected' : "" ?> >Tous les mois</option>
			</select>
		</div>

		<div class="five columns offset-by-two">
			<label>Intervalle en seconde entre les prises de vue (0-86400):</label><div id="texteIntervalle" ><?= $settingsInterface -> snapshot_interval ?></div>
			<input class="u-full-width" type="range" min="0" max="86400" value="<?= $settingsInterface -> snapshot_interval ?>" placeholder="<?= $settingsInterface -> snapshot_interval ?>" name="snapshot_interval" id="snapshot_interval" oninput="updateSnapInterval(this.value);">
		</div>
	</div>
