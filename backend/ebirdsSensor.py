...
class APISqlite3:
    ...
    def dbInsert(self, key, value):
        ...
        try:
           cursor.execute('''INSERT INTO {} (?) VALUES (?)'''.format(self._dbTable),( key, value))
...
class DBManager:
    ...
    def setAPI(self):
        return globals()[self._APIlink](self._dbFile, self.dbTable)
...
class DHTSensor:
    ...
    def __init__(self, sensorType):
        if (sensorType == dhtSensor.DHT11 or sensorType == dhtSensor.DHT22):
    ...
    def read(self, pinNum):
        ...
        while count < 5:
            humidity, temperature = dhtSensor.read(self._type, pinNum)
...
if __name__ == "__main__":
    ...
    DB = "/var/www/ebirds.db"
    TABLE = "meteo"
    DHT = 11

    dbCapteur = DBManager(DB, TABLE, "sqlite3").setAPI()
    readOut = DHTSensor(DHT, 20).read()
